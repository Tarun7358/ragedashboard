...
export class GuildQueue {
  ...
  public async updatePanel(client: any) {
    if (!this.textChannelId || !client) {
      console.log(`[Music Panel] Skipping: textChannelId=${this.textChannelId}, client=${!!client}`);
      return;
    }

    try {
      const channel = await client.channels.fetch(this.textChannelId).catch((e: any) => {
        console.error(`[Music Panel] Failed to fetch channel ${this.textChannelId}:`, e?.message || e);
        return null;
      });
      if (!channel) {
        console.log(`[Music Panel] Channel ${this.textChannelId} not found.`);
        return;
      }
      ...
      if (this.panelMessageId) {
        ...
        if (msg) {
          await msg.edit({ embeds: [embed], components }).catch((e: any) => {
            console.error('[Music Panel] Failed to edit panel message:', e?.message || e);
          });
      ...
      const newMsg = await channel.send({ embeds: [embed], components }).catch((e: any) => {
        console.error('[Music Panel] Failed to send panel message to channel', this.textChannelId, ':', e?.message || e);
        return null;
      });
      if (newMsg) {
        this.panelMessageId = newMsg.id;
        console.log(`[Music Panel] Panel sent successfully to channel ${this.textChannelId}`);
      }
      await this.broadcastState();
